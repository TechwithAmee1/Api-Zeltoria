...
const fbdl02 = await fbdl2(videoUrl);
const fbdl = await fbdl1(videoUrl);
    ...
    axios.get(videoUrl, { headers }).then(({ data }) => {
      ...
      if (sdMatch && sdMatch[1]) {
        const result = {
          ...
          Aud: fbdl02.Audio || null,
          title: titleMatch && titleMatch[1] ? parseString(titleMatch[1]) : data.match(/<title>(.*?)<\/title>/)?.[1] ?? fbdl02.title,
          duration: fbdl.meta.duration || "",
