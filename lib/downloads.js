...
const duration = (await fbdl1(videoUrl)).meta.duration;
const title = (await fbdl2(videoUrl)).Title;
const aud = (await fbdl2(videoUrl)).Audio;
    ...
    axios.get(videoUrl, { headers }).then(({ data }) => {
      ...
      if (sdMatch && sdMatch[1]) {
        const result = {
          ...
          Aud: aud || null,
          title: titleMatch && titleMatch[1] ? parseString(titleMatch[1]) : data.match(/<title>(.*?)<\/title>/)?.[1] ?? title,
          duration: duration || "",
